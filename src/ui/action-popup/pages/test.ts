...
function betweenTimes(date: any, start: any, end: any): boolean {
...
export const computedWorkTime = function (
  ...
  // 是否跨天
  const isCrossing = crossing;

  const endDate = new Date(today);
  const startDate = new Date(isCrossing ? yesterday : today);
  ...
  // 如果位于午休时间内，设为午休结束时间
  if (
    computedStartTime === noonBeakTime.startTime ||
    computedStartTime === noonBeakTime.endTime ||
    betweenTimes(computedStartTime, noonBeakTime.startTime, noonBeakTime.endTime)
  ) {
    computedStartTime = noonBeakTime.endTime;
  }
  ...
  if (
    ...
    (computedEndTime === nexDayNoonBeakTime.startTime ||
      computedEndTime === nexDayNoonBeakTime.endTime ||
      betweenTimes(computedEndTime, nexDayNoonBeakTime.startTime, nexDayNoonBeakTime.endTime))
  ) {
    computedEndTime = nexDayNoonBeakTime.startTime;
  } else {
    // 如果位于午休时间内，设为午休开始时间
  ...
  if (
    ...
    breakDurationInMinutes = Number(seasonHour) * 60 + Number(seasonMinutes);
  }
  ...
  // 如果开始时间或结束时间位于午休时间内，设为 0
  if (
    (computedStartTime >= noonBeakTime.startTime && computedStartTime <= noonBeakTime.endTime) ||
    (computedEndTime >= noonBeakTime.startTime && computedEndTime <= noonBeakTime.endTime)
  ) {
    breakDurationInMinutes = 0;
  }
